...
## Релизный игровой цикл: первая итерация

- Меню явно загружает свои компоненты, поэтому старт не зависит от ещё не
  прогретого кэша глобальных классов Godot. Если WAV не успел импортироваться,
  игра запускается без радио вместо ошибки загрузки.
- Семь архетипов пассажиров: обычный, пенсионер, школьник, спешащий,
  недовольный, работяга и пассажир с сумкой. У них свои реплики при посадке
  и реакции на резкую езду.
- События ограничены тремя за рейс: реплики и деньги остаются заметными,
  не перекрывая управление постоянными всплывающими окнами.
- Появилось состояние маршрутки. Удары снижают разгон и скорость умеренно;
  в гараже видны процент состояния и стоимость ремонта.
- «Рейс дня» меняется по дате: час пик, аккуратный рейс или день зарплаты.
  Он меняет пассажиропоток/выручку и даёт бонус на экране результатов.

Проверено после чистого импорта: 35 проверок базового сценария, два полных
рейса обеих машин, 118 регрессионных проверок управления/города/трафика и
9 проверок дорожного трафика прошли без ошибок.
