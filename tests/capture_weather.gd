extends SceneTree
## Actual renderer views of overcast weather and city traffic.
...
func run() -> void:
	...
	await shot("overcast_quarters")
	game.camera_rig.camera.global_position=Vector3(88,3.0,-138)
	game.camera_rig.camera.look_at(Vector3(113,4.0,-139))
	await shot("traffic_junction")
	game.camera_rig.camera.global_position=Vector3(2.5,3,-80)
	game.camera_rig.camera.look_at(Vector3(2.5,17,-105))
	await shot("overcast_sky")
	quit()
